...
- API 설정 화면에서 값을 저장한 뒤에는 개발 서버를 다시 시작해야 `.env.local` 값이 반영된다.
- 직접 검색 방식에서는 `OPEN_LAW_OC`를 우선 사용한다. `OPEN_LAW_OC`가 비어 있고 `LAW_API_KEY`만 있으면 같은 값을 OC로 간주해 검색한다.
- `법률정보 후보 찾기`는 질문을 직접 쓰지 않아도 상황 카드, 업종, 현재 단계만으로 실행할 수 있다.
- 실제 API 키나 OC 값은 README, 이슈, 채팅창, 화면 캡처에 노출하지 않는다.
