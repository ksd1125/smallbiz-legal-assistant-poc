...
### 데모 시나리오 데이터 수급 현실성 보강

- `docs/demo-scenario.html`의 서비스 설명 패널에 데이터 수급 현실성을 추가했다.
- 자동 수급 가능 자료, 반자동 수급·검수 자료, 답변 제한 영역을 구분해 목업이 실서비스 가능 범위를 과장하지 않도록 정리했다.
- 지방행정 인허가 데이터는 기존 LOCALDATA 폐쇄 공지를 반영해 공공데이터포털 전환 확인 후 연결하도록 표시했다.
- 내부 데이터 흐름에 하이브리드 검색, 메타데이터 필터, 질의 재작성, 출처 추적, 평가 세트, 사람 검수 구조를 반영했다.
- `tests/demo-scenario-static.test.js`에 해당 설명 문구가 유지되는지 확인하는 정적 테스트를 추가했다.

### 데모 시나리오 버튼 반응 보강

- 왼쪽 시나리오 버튼을 눌러도 현재 화면이 바뀌지 않아 사용자가 무반응처럼 느끼는 구조를 확인했다.
- 시나리오 버튼 클릭 시 선택된 시나리오 답변이 보이는 `Codex형 일반 질의` 화면으로 전환되도록 수정했다.
- 칩 버튼과 심화 입력 화면의 업종 선택은 기존처럼 현재 화면 맥락을 유지하도록 했다.
- `tests/demo-scenario-static.test.js`에 사이드바 시나리오 버튼이 보이는 답변 화면으로 이동하는지 확인하는 테스트를 추가했다.

### 일반 질의·심화 점검 UI 명확화

- 일반 질의 화면을 좌측 `질문` 패널과 우측 `답변` 패널로 분리해 입력과 결과가 한눈에 구분되도록 수정했다.
- 일반 답변 영역에 요약, 관련 가능 분야, 다음 확인할 일을 별도 영역으로 나눠 표시했다.
- 업종·상황 심화 점검 화면을 `입력 조건` 패널과 `심화 답변` 패널로 재구성했다.
- 심화 답변 영역에 시나리오 요약, 관련 분야, 체크리스트, 추가 질문, 상담 연결을 순서대로 배치했다.
- `tests/demo-scenario-static.test.js`에 질의/답변 UI 구조가 유지되는지 확인하는 테스트를 추가했다.

### 심화 점검 입력 피로도 축소

- 심화 점검의 기본 입력을 `업종군`과 `질문` 중심의 필수 입력으로 줄였다.
- 사업 단계, 직원 수, 사업장 형태, 위험요소는 `선택 입력`으로 접어 두고 필요할 때만 열도록 수정했다.
- 답변 영역에 `추정한 상황`을 추가해 사용자가 모든 항목을 채우지 않아도 서비스가 어떤 전제로 답하는지 보이게 했다.
- 기존 `추가 질문` 영역을 `더 정확히 답하기 위한 질문`으로 바꿔, 부족한 정보는 답변 이후 질의로 보완하는 흐름을 반영했다.
- `tests/demo-scenario-static.test.js`에 필수 입력, 선택 입력, 접기 버튼, 후속 질문 영역을 확인하는 테스트를 추가했다.

### 4대 AI 통합 허브 목업 추가

- `C:\Users\USER\Desktop\codex\projects\프로젝트\AI 서비스 통합 및 신규 서비스 구축.md`의 내용을 확인했다.
- 정책, 상권, 법률, 통계 4개 AI를 단일 입력창과 오케스트레이터로 묶는 `docs/integrated-ai-hub.html` 목업을 추가했다.
- 통합 화면에는 단일 입력창, 라우팅 단계, 통합 답변, 4대 AI 요약 카드, 세부 보기 패널을 배치했다.
- 세부 패널에는 정책 AI, 상권 AI, 법률 AI, 통계 AI별 서비스 내용, 데이터·엔진, 출처를 표시했다.
- Fallback, 보안·신뢰성, 품질 기준, 동적 UI, Text-to-SQL, RAG 요구사항을 화면에서 설명하도록 구성했다.
- `tests/integrated-ai-mockup-static.test.js`를 추가하고 `npm.cmd run test:static` 검증 대상에 포함했다.

### 통합 허브와 도메인별 전문 대시보드 재구성

- 통합 화면을 깊은 답변 영역이 아니라 브랜드·업종·지역을 해석하고 세부 대시보드로 안내하는 화면으로 재정의했다.
- `파리바게트` 예시를 사용해 사용자 표현을 상권, 정책, 법률, 통계 기준으로 나누는 `업종·브랜드 매핑` 패널을 추가했다.
- 상권 세부는 지역·업종 기반 차트와 질의로 기간·비교지역·업종 기준을 바꾸는 `상권 전문 대시보드`로 구성했다.
- 정책 세부는 공고 나열이 아니라 사용자 조건과 공고 기준을 맞춰가는 `정책 조건 매칭 대시보드`로 구성했다.
- 법률 세부는 기존 방향을 유지해 상황 유형, 현재 단계, 준비자료, 답변 경계를 보여주는 `법률 상황 체크 대시보드`로 정리했다.
- 통계 세부는 KOSIS 수치 생성이 아니라 공식 통계표, 설문·결과보고서, 조사자료를 찾는 `통계·보고서 탐색 대시보드`로 구성했다.
- 원천 데이터 수집, 파싱, 인덱싱, 라우팅, 에이전트, 검증, UI 렌더링까지 이어지는 내부 처리 흐름 패널을 추가했다.
